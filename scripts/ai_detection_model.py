...
def load_model(model_directory="desklib/ai-text-detector-v1.01"):
...
def detect_ai_text(text, model_directory="desklib/ai-text-detector-v1.01", threshold=0.5):
    """
    ...
    Returns:
        dict: A dictionary with probability and prediction
    """
    model, tokenizer, device = load_model(model_directory)
    probability, label = predict_single_text(text, model, tokenizer, device, threshold=threshold)
    
    return {
        "probability": float(probability),
        "is_ai_generated": bool(label == 1)
    }
